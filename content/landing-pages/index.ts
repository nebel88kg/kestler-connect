import type { LandingPage } from "../types";

export const googleAdsFuerHandwerker: LandingPage = {
  slug: "google-ads-fuer-handwerker",
  path: "/leistungen/google-ads/google-ads-fuer-handwerker",
  category: "google-ads",
  meta: {
    title: "Google Ads für Handwerker",
    description:
      "Mehr Aufträge für Ihren Handwerksbetrieb mit professionellen Google Ads Kampagnen. Gezielte Leadgenerierung für Handwerk und Gewerbe.",
    keywords: ["google ads handwerker", "handwerker werbung", "google ads gewerbe"],
  },
  hero: {
    headline: "Google Ads für Handwerker – Mehr Aufträge, weniger Leerlauf",
    subheadline:
      "Wir bringen Ihnen qualifizierte Anfragen von Kunden, die genau jetzt einen Handwerker suchen. Keine Streuverluste, messbare Ergebnisse.",
  },
  problem: {
    title: "Kennen Sie diese Herausforderungen?",
    points: [
      "Zu wenig planbare Aufträge trotz guter Arbeit",
      "Hohe Kosten bei klassischer Zeitungswerbung ohne messbare Ergebnisse",
      "Konkurrenz auf Google sichtbar, Sie nicht",
      "Keine Zeit, sich selbst um Online-Marketing zu kümmern",
      "Bisherige Google Ads Versuche ohne Erfolg",
    ],
  },
  solution: {
    title: "Unsere Lösung für Handwerksbetriebe",
    content:
      "Bei Kestler Connect verstehen wir die Handwerksbranche. Wir erstellen Google Ads Kampagnen, die genau dann erscheinen, wenn jemand in Ihrer Region nach Ihrer Leistung sucht. Mit lokalen Keywords, optimierten Landingpages und transparentem Reporting sehen Sie genau, was jeder Euro bringt.",
  },
  benefits: [
    {
      title: "Lokale Sichtbarkeit",
      description: "Ihre Anzeigen erscheinen genau in Ihrem Einzugsgebiet – bei den richtigen Kunden zur richtigen Zeit.",
      icon: "map",
    },
    {
      title: "Qualifizierte Anfragen",
      description: "Keine Klick-Käufer, sondern echte Interessenten mit konkretem Bedarf an Ihrer Leistung.",
      icon: "users",
    },
    {
      title: "Messbare Ergebnisse",
      description: "Transparentes Reporting: Sie sehen Anfragen, Kosten und ROI auf einen Blick.",
      icon: "chart",
    },
    {
      title: "Kein Vertragschaos",
      description: "Klare Kommunikation, faire Konditionen und volle Kontrolle über Ihr Werbebudget.",
      icon: "shield",
    },
    {
      title: "Branchen-Know-how",
      description: "Wir kennen die Besonderheiten des Handwerks – von SHK über Elektro bis Dachdecker.",
      icon: "wrench",
    },
    {
      title: "Schneller Start",
      description: "Innerhalb weniger Tage live – erste Anfragen oft schon in der ersten Woche.",
      icon: "rocket",
    },
  ],
  references: [
    {
      title: "SHK-Betrieb in NRW",
      slug: "handwerksbetrieb-shk",
      excerpt: "120% mehr Anfragen in 3 Monaten durch lokale Google Ads Kampagnen.",
    },
    {
      title: "Elektrobetrieb Mitteldeutschland",
      slug: "handwerksbetrieb-elektro",
      excerpt: "Kosten pro Anfrage um 40% gesenkt durch Conversion-Optimierung.",
    },
  ],
  process: [
    { step: 1, title: "Analyse", description: "Wir analysieren Ihren Markt, Wettbewerb und bisherige Online-Präsenz." },
    { step: 2, title: "Strategie", description: "Keyword-Recherche und Kampagnenstruktur für maximale Relevanz." },
    { step: 3, title: "Umsetzung", description: "Anzeigen, Landingpages und Tracking werden professionell eingerichtet." },
    { step: 4, title: "Optimierung", description: "Laufende A/B-Tests und Gebotsanpassungen für bessere Ergebnisse." },
    { step: 5, title: "Reporting", description: "Monatliche Reports mit klaren Handlungsempfehlungen." },
  ],
  faq: [
    {
      question: "Ab welchem Budget lohnen sich Google Ads für Handwerker?",
      answer: "Für lokale Handwerksbetriebe empfehlen wir ein Mindestbudget von 500–800 € monatlich. Damit erreichen Sie genug Reichweite für aussagekräftige Ergebnisse.",
    },
    {
      question: "Wie schnell kommen die ersten Anfragen?",
      answer: "Nach dem Go-live können erste Anfragen innerhalb von 24–48 Stunden eintreffen. Optimale Ergebnisse sehen wir typischerweise nach 4–6 Wochen.",
    },
    {
      question: "Brauche ich eine eigene Webseite?",
      answer: "Ideal ist eine eigene Webseite mit einer optimierten Landingpage. Falls nicht vorhanden, erstellen wir diese für Sie.",
    },
    {
      question: "Gibt es Vertragslaufzeiten?",
      answer: "Wir arbeiten mit fairen Konditionen ohne lange Bindung. Unser Ziel: Ergebnisse, die Sie überzeugen.",
    },
    {
      question: "Kann ich mein bestehendes Google Ads Konto nutzen?",
      answer: "Ja, wir können Ihr bestehendes Konto übernehmen und optimieren oder ein neues professionell aufsetzen.",
    },
  ],
};

export const leadgewinnung: LandingPage = {
  slug: "leadgewinnung",
  path: "/leistungen/leadgewinnung",
  category: "leadgewinnung",
  meta: {
    title: "Leadgewinnung",
    description:
      "Qualifizierte Anfragen für Ihr Unternehmen – mit Google Ads, Meta Ads und Landingpages. Messbar, planbar und ohne Streuverluste.",
    keywords: ["leadgewinnung", "leads generieren", "anfragen gewinnen", "performance marketing"],
  },
  hero: {
    headline: "Leadgewinnung – Anfragen, die zu Kunden werden",
    subheadline:
      "Wir bringen qualifizierte Interessenten zu Ihnen: über die richtigen Kanäle, mit klaren Botschaften und Landingpages, die konvertieren.",
  },
  problem: {
    title: "Kennen Sie das?",
    points: [
      "Zu wenig planbare Anfragen trotz guter Leistung",
      "Werbung ohne messbaren Erfolg oder zu hohe Kosten pro Lead",
      "Website-Besucher, die abspringen, bevor sie Kontakt aufnehmen",
      "Unklare Zuständigkeit: Wer kümmert sich um Kampagnen und Nachverfolgung?",
      "Kein Überblick, welcher Kanal wirklich Aufträge bringt",
    ],
  },
  solution: {
    title: "Leadgewinnung mit System",
    content:
      "Wir verbinden Strategie, Kampagnen und Conversion: Google Ads und Meta Ads für Reichweite und Nachfrage, starke Landingpages und Formulare für die Anfrage, Tracking und Reporting für Transparenz. Sie sehen jederzeit Kosten, Qualität und Ergebnis – und skalieren, was funktioniert.",
  },
  benefits: [
    { title: "Kanalmix mit Fokus", description: "Google, Meta und Landingpages – abgestimmt auf Ihre Zielgruppe und Region.", icon: "target" },
    { title: "Qualifizierte Anfragen", description: "Formulare und Targeting so gebaut, dass echte Interessenten ankommen.", icon: "form" },
    { title: "Conversion-stark", description: "Seiten und Creatives, die aus Klicks Anfragen machen.", icon: "rocket" },
    { title: "Messbar", description: "Kosten pro Lead, Quellen und ROI transparent im Blick.", icon: "chart" },
    { title: "Skalierbar", description: "Erfolgreiche Kampagnen gezielt ausbauen – ohne Rätselraten.", icon: "scale" },
    { title: "Alles aus einer Hand", description: "Strategie, Umsetzung und Optimierung bei einem Ansprechpartner.", icon: "star" },
  ],
  references: [
    { title: "Lokaler Dienstleister", slug: "handwerksbetrieb-shk", excerpt: "80 qualifizierte Leads in 2 Monaten zu unter 25 € pro Lead." },
    { title: "Immobilienmakler", slug: "immobilienmakler-regional", excerpt: "45% mehr Besichtigungstermine durch Lead-Kampagnen." },
  ],
  process: [
    { step: 1, title: "Analyse", description: "Ziele, Zielgruppe, bisherige Kanäle und Potenzial klären." },
    { step: 2, title: "Setup", description: "Kampagnen, Landingpages und Tracking einrichten." },
    { step: 3, title: "Launch", description: "Start mit ersten Anfragen und Datengrundlage." },
    { step: 4, title: "Optimierung", description: "Creatives, Targeting und Conversion laufend verbessern." },
    { step: 5, title: "Skalierung", description: "Budget und Reichweite dort erhöhen, wo die Qualität stimmt." },
  ],
  faq: [
    { question: "Was kostet Leadgewinnung?", answer: "Agenturkosten typischerweise ab 500 €/Monat plus Werbebudget. Das Budget richten wir nach Ihren Zielen und dem Markt aus." },
    { question: "Google Ads, Meta Ads oder beides?", answer: "Das hängt von Ihrer Zielgruppe ab. Oft funktioniert eine Kombination am besten – wir empfehlen den sinnvollsten Einstieg." },
    { question: "Wie qualifiziert sind die Leads?", answer: "Über Targeting, Formularfragen und Landingpage-Texte steuern wir Qualität. Kriterien legen wir gemeinsam fest." },
    { question: "Wie schnell sehe ich Ergebnisse?", answer: "Erste Anfragen oft innerhalb weniger Tage nach Launch. Stabile Kosten und Qualität entstehen über laufende Optimierung." },
    { question: "Gibt es Mindestlaufzeiten?", answer: "Nein. Wir überzeugen durch Ergebnisse, nicht durch Vertragsbindung." },
  ],
};

export const mitgliedergewinnungGolf: LandingPage = {
  slug: "mitgliedergewinnung-golfclubs",
  path: "/leistungen/meta-ads/mitgliedergewinnung-golfclubs",
  category: "meta-ads",
  meta: {
    title: "Mitgliedergewinnung für Golfclubs",
    description:
      "Neue Golf-Mitglieder gewinnen mit gezielten Marketing-Kampagnen. Erfahrung mit Golfclub Raffelberg und weiteren Clubs.",
    keywords: ["golfclub mitglieder gewinnen", "golf marketing", "golfclub werbung"],
  },
  hero: {
    headline: "Mitgliedergewinnung für Golfclubs – Volle Mitgliederkontingente",
    subheadline:
      "Wir helfen Golfclubs, neue Mitglieder zu gewinnen – mit bewährten Strategien aus der Praxis, u.a. für den Golfclub Raffelberg.",
  },
  problem: {
    title: "Herausforderungen für Golfclubs",
    points: [
      "Sinkende Mitgliederzahlen trotz attraktiver Anlage",
      "Junge Zielgruppen erreichen traditionelle Werbung nicht",
      "Hohe Kosten für Print-Anzeigen ohne messbare Ergebnisse",
      "Konkurrenz durch andere Freizeitangebote",
      "Kein internes Marketing-Know-how im Verein",
    ],
  },
  solution: {
    title: "Marketing, das Golfspieler anspricht",
    content:
      "Wir kombinieren Google Ads, Meta Kampagnen und Social Media Content, um Golfer in Ihrer Region anzusprechen. Von Schnuppergolf-Aktionen bis Platzreifekursen – wir wissen, was funktioniert.",
  },
  benefits: [
    { title: "Golf-Branchenexpertise", description: "Erfahrung mit Golfclub Raffelberg und weiteren Clubs.", icon: "golf" },
    { title: "Zielgruppen-Targeting", description: "Erreichen Sie Golfer und Interessenten in Ihrer Region.", icon: "target" },
    { title: "Schnuppergolf-Kampagnen", description: "Bewährte Kampagnen für Schnupperangebote und Events.", icon: "flag" },
    { title: "Social Media Präsenz", description: "Professioneller Auftritt auf Instagram und Facebook.", icon: "social" },
    { title: "Platzreife-Marketing", description: "Gezielte Kampagnen für Platzreifekurse und Jugendförderung.", icon: "education" },
    { title: "Messbare Ergebnisse", description: "Anmeldungen und Schnupperanfragen transparent tracken.", icon: "chart" },
  ],
  references: [
    { title: "Golfclub Raffelberg", slug: "golfclub-raffelberg", excerpt: "Erfolgreiche Mitgliedergewinnung und digitale Präsenz." },
    { title: "Regionaler Golfclub", slug: "golfclub-regional", excerpt: "40% mehr Schnupperanfragen durch Meta-Kampagnen." },
  ],
  process: [
    { step: 1, title: "Club-Analyse", description: "Mitgliederstruktur, Zielgruppe und Wettbewerb analysieren." },
    { step: 2, title: "Kampagnen-Strategie", description: "Maßgeschneiderte Strategie für Ihren Club entwickeln." },
    { step: 3, title: "Content & Creatives", description: "Professionelle Bilder, Videos und Anzeigen erstellen." },
    { step: 4, title: "Kampagnen-Launch", description: "Google Ads, Meta Ads und Social Media starten." },
    { step: 5, title: "Optimierung", description: "Laufende Anpassung für maximale Mitgliedergewinnung." },
  ],
  faq: [
    { question: "Wie gewinnen Golfclubs neue Mitglieder online?", answer: "Durch die Kombination aus Google Ads für aktive Suchende, Meta Ads für Interessenten und professionellem Social Media Content." },
    { question: "Was kostet Marketing für einen Golfclub?", answer: "Je nach Umfang ab 800 €/Monat für Agenturleistungen plus Werbebudget. Wir erstellen gerne ein individuelles Angebot." },
    { question: "Habt ihr Erfahrung mit Golfclubs?", answer: "Ja, unter anderem mit dem Golfclub Raffelberg. Wir verstehen die Besonderheiten der Golfbranche." },
    { question: "Können wir Schnuppergolf-Aktionen bewerben?", answer: "Absolut. Schnuppergolf-Kampagnen sind einer unserer bewährtesten Ansätze für die Mitgliedergewinnung." },
    { question: "Wie lange dauert es bis zum ersten Erfolg?", answer: "Erste Anfragen oft innerhalb der ersten 2 Wochen. Optimale Ergebnisse nach 2–3 Monaten kontinuierlicher Kampagnen." },
  ],
};

export const localSeo: LandingPage = {
  slug: "local-seo",
  path: "/leistungen/seo/local-seo",
  category: "seo",
  meta: {
    title: "Local SEO – Lokal gefunden werden",
    description:
      "Local SEO für lokale Unternehmen und Handwerk. Bei Google Maps und lokalen Suchanfragen ganz oben erscheinen.",
    keywords: ["local seo", "lokale seo", "google maps optimierung"],
  },
  hero: {
    headline: "Local SEO – Werden Sie in Ihrer Region gefunden",
    subheadline:
      "Wenn jemand in Ihrer Stadt nach Ihrer Leistung sucht, soll Ihr Unternehmen als Erstes erscheinen. Genau dafür ist Local SEO da.",
  },
  problem: {
    title: "Ohne Local SEO verlieren Sie Kunden",
    points: [
      "Konkurrenten erscheinen bei 'Handwerker + Stadt' vor Ihnen",
      "Ihr Google Business Profil ist unvollständig oder veraltet",
      "Keine oder schlechte Bewertungen auf Google",
      "Ihre Webseite ist nicht für lokale Suchanfragen optimiert",
      "Sie werden auf Google Maps nicht angezeigt",
    ],
  },
  solution: {
    title: "Local SEO mit System",
    content:
      "Wir optimieren Ihr Google Business Profil, Ihre Webseite und Ihre lokalen Verzeichniseinträge. Das Ergebnis: Mehr Sichtbarkeit bei lokalen Suchanfragen und mehr Anrufe von Kunden in Ihrer Nähe.",
  },
  benefits: [
    { title: "Google Maps", description: "Top-Platzierungen in der lokalen Pack-Liste.", icon: "map" },
    { title: "Business Profil", description: "Vollständig optimiertes Google Business Profil.", icon: "building" },
    { title: "Bewertungsmanagement", description: "Strategien für mehr und bessere Google-Bewertungen.", icon: "star" },
    { title: "Lokale Keywords", description: "Webseite optimiert für 'Leistung + Stadt' Suchanfragen.", icon: "search" },
    { title: "NAP-Konsistenz", description: "Einheitliche Unternehmensdaten im gesamten Web.", icon: "check" },
    { title: "Langfristig", description: "Nachhaltige Sichtbarkeit ohne laufende Werbekosten.", icon: "trending" },
  ],
  references: [
    { title: "SHK-Betrieb in NRW", slug: "handwerksbetrieb-shk", excerpt: "Position 1 bei 12 lokalen Keywords innerhalb von 4 Monaten." },
    { title: "Lokaler Dienstleister", slug: "handwerksbetrieb-elektro", excerpt: "300% mehr Google Maps Aufrufe nach Local SEO Optimierung." },
  ],
  process: [
    { step: 1, title: "Local Audit", description: "Analyse Ihrer aktuellen lokalen Sichtbarkeit." },
    { step: 2, title: "Optimierung", description: "Google Business Profil, Webseite und Verzeichnisse optimieren." },
    { step: 3, title: "Content", description: "Lokale Inhalte und Landingpages erstellen." },
    { step: 4, title: "Bewertungen", description: "System für mehr Google-Bewertungen aufbauen." },
    { step: 5, title: "Monitoring", description: "Rankings und Sichtbarkeit kontinuierlich überwachen." },
  ],
  faq: [
    { question: "Wie lange dauert Local SEO?", answer: "Erste Verbesserungen nach 4–8 Wochen. Signifikante Rankings nach 3–6 Monaten." },
    { question: "Brauche ich eine Webseite für Local SEO?", answer: "Ja, eine optimierte Webseite ist Grundvoraussetzung für nachhaltiges Local SEO." },
    { question: "Was ist der Unterschied zu Google Ads?", answer: "Local SEO ist langfristig und kostenlos nach der Optimierung. Google Ads bringt sofortige Ergebnisse gegen laufende Kosten." },
    { question: "Optimiert ihr auch Google Business Profile?", answer: "Ja, das Google Business Profil ist ein zentraler Bestandteil unserer Local SEO Strategie." },
    { question: "Für welche Branchen eignet sich Local SEO?", answer: "Ideal für alle lokalen Unternehmen: Handwerk, Gastronomie, Ärzte, Anwälte, Golfclubs und mehr." },
  ],
};

export const mitarbeitergewinnung: LandingPage = {
  slug: "mitarbeitergewinnung",
  path: "/leistungen/mitarbeitergewinnung",
  category: "mitarbeitergewinnung",
  meta: {
    title: "Mitarbeitergewinnung",
    description:
      "Fachkräfte und Bewerber aktiv gewinnen – mit Social Recruiting, Google Ads und starkem Employer Branding. Für Handwerk, Vereine und lokale Unternehmen.",
    keywords: ["mitarbeitergewinnung", "social recruiting", "fachkräfte finden", "employer branding"],
  },
  hero: {
    headline: "Mitarbeitergewinnung – die richtigen Leute aktiv ansprechen",
    subheadline:
      "Statt auf Bewerbungen zu warten, finden wir Fachkräfte und Talente dort, wo sie unterwegs sind – mit klarer Ansprache, starken Creatives und messbaren Kampagnen.",
  },
  problem: {
    title: "Fachkräfte werden nicht von allein gefunden",
    points: [
      "Stellenanzeigen auf Jobportalen bringen kaum passende Bewerbungen",
      "Gute Kandidaten sind oft nicht aktiv auf der Suche",
      "Personalvermittler sind teuer – ohne Garantie auf Passung",
      "Lange Vakanzen bremsen Aufträge und Wachstum",
      "Ihr Betrieb wirkt online nicht als attraktiver Arbeitgeber",
    ],
  },
  solution: {
    title: "Recruiting, das zu Ihnen passt",
    content:
      "Wir verbinden Employer Branding mit Social Recruiting und Google Ads: authentische Einblicke in Ihren Betrieb, gezielte Ansprache in der Region und ein Bewerbungsprozess, der einfach funktioniert. So kommen qualifizierte Bewerbungen – planbarer und oft günstiger als klassische Vermittlung.",
  },
  benefits: [
    { title: "Social Recruiting", description: "Bewerber auf Meta und Social Media aktiv ansprechen.", icon: "social" },
    { title: "Google Ads Jobs", description: "Sichtbarkeit bei Menschen, die aktiv nach Stellen suchen.", icon: "search" },
    { title: "Employer Branding", description: "Ihren Betrieb als Arbeitgeber klar und glaubwürdig positionieren.", icon: "star" },
    { title: "Video & Content", description: "Team, Alltag und Kultur sichtbar machen – das überzeugt.", icon: "video" },
    { title: "Schnelle Impulse", description: "Erste Bewerbungen oft schon in den ersten Wochen.", icon: "rocket" },
    { title: "Messbar", description: "Kosten pro Bewerbung und Kanaltransparenz statt Bauchgefühl.", icon: "chart" },
  ],
  references: [
    { title: "SHK-Betrieb in NRW", slug: "handwerksbetrieb-shk", excerpt: "3 qualifizierte Fachkräfte in 6 Wochen über Social Recruiting." },
    { title: "Elektrobetrieb", slug: "handwerksbetrieb-elektro", excerpt: "Azubi-Kampagne mit 12 Bewerbungen in 4 Wochen." },
  ],
  process: [
    { step: 1, title: "Bedarfsanalyse", description: "Stellen, Profil, Region und Arbeitgeber-Stärken klären." },
    { step: 2, title: "Positionierung", description: "Botschaften, Creatives und Bewerbungsstrecke aufsetzen." },
    { step: 3, title: "Kampagnen-Launch", description: "Social Recruiting und ggf. Google Ads starten." },
    { step: 4, title: "Bewerber-Flow", description: "Anfragen qualifizieren und an Sie weiterleiten." },
    { step: 5, title: "Optimierung", description: "Ansprache und Kanäle für bessere Passung nachschärfen." },
  ],
  faq: [
    { question: "Wie lange dauert Mitarbeitergewinnung?", answer: "Erste Bewerbungen oft innerhalb von 1–2 Wochen. Bis zur Einstellung typischerweise einige Wochen – abhängig von Profil und Markt." },
    { question: "Was kostet das?", answer: "Agenturkosten typischerweise ab 600 €/Monat plus Werbebudget. Oft günstiger als Personalvermittler mit Erfolgsprovision." },
    { question: "Für wen eignet sich das?", answer: "Besonders für Handwerk, lokale Dienstleister, Vereine und Betriebe mit Fachkräfte- oder Azubi-Bedarf." },
    { question: "Erstellt ihr Stellenanzeigen und Videos?", answer: "Ja – Texte, Bilder und Videos für Ihre Recruiting-Kampagnen gehören dazu." },
    { question: "Geht das auch für Azubis?", answer: "Ja. Azubi-Kampagnen mit eigener Ansprache sind ein fester Bestandteil unseres Angebots." },
  ],
};

export const marketingWorkshopStartups: LandingPage = {
  slug: "marketing-workshop-startups",
  path: "/leistungen/workshops/marketing-workshop-startups",
  category: "workshops",
  meta: {
    title: "Marketing-Workshop für Startups",
    description:
      "Kompakter Marketing-Workshop (5–6 Stunden) für Startups und junge Unternehmen – ohne großes Budget, mit klarem Fokus und sofort umsetzbaren Schritten.",
    keywords: [
      "marketing workshop startup",
      "marketing workshop gründern",
      "online marketing workshop",
      "startup marketing",
    ],
  },
  hero: {
    headline: "Marketing-Workshop für Startups – Klarheit in 5–6 Stunden",
    subheadline:
      "Sie wollen sichtbar werden, haben aber noch kein Budget für ein komplettes Marketing-Setup? In einem kompakten Workshop bekommen Sie Struktur, Prioritäten und konkrete nächste Schritte.",
  },
  problem: {
    title: "Typisch für den Start",
    points: [
      "Kein großes Marketing-Budget – und trotzdem Druck, sichtbar zu werden",
      "Zu viele Kanäle, zu wenig Klarheit, womit man wirklich starten sollte",
      "Content und Ads werden ausprobiert, ohne System dahinter",
      "Keine Zeit und kein Team für eine laufende Agentur-Betreuung",
      "Unsicherheit, was sich für ein junges Unternehmen überhaupt lohnt",
    ],
  },
  solution: {
    title: "Ein kompakter Einstieg statt Vollprogramm",
    content:
      "Dieser Workshop ist bewusst schlank gehalten: In fünf bis sechs Stunden arbeiten wir an Positionierung, Zielgruppe, Kanälen und einem realistischen Startplan. Sie gehen mit Prioritäten und Handwerkszeug nach Hause – nicht mit einer Agenturrechnung für Monate im Voraus. Ideal für Startups und junge Unternehmen, die erst einmal selbst Fahrt aufnehmen wollen.",
  },
  benefits: [
    {
      title: "5–6 Stunden Fokus",
      description: "Ein kompakter Block statt langer Projekte – klar, intensiv und ergebnisorientiert.",
      icon: "rocket",
    },
    {
      title: "Budgetfreundlich",
      description: "Für Teams, die noch kein volles Marketing-Budget haben, aber trotzdem professionell starten wollen.",
      icon: "shield",
    },
    {
      title: "Sofort anwendbar",
      description: "Konkrete To-dos, Kanäle und Botschaften – am nächsten Tag umsetzbar.",
      icon: "check",
    },
    {
      title: "Prioritäten statt Overwhelm",
      description: "Wir filtern, was jetzt zählt – und was Sie später skalieren können.",
      icon: "target",
    },
    {
      title: "Praxis statt Folien",
      description: "Arbeit an Ihrem echten Setup: Zielgruppe, Angebote, Kanäle und nächste Schritte.",
      icon: "wrench",
    },
    {
      title: "Optionaler Anschluss",
      description: "Wenn Sie später skalieren wollen, können wir aus dem Workshop in eine laufende Betreuung übergehen.",
      icon: "trending",
    },
  ],
  references: [
    {
      title: "Lokaler Dienstleister",
      slug: "handwerksbetrieb-shk",
      excerpt: "Von unklarer Sichtbarkeit zu planbaren Anfragen – mit klarer Priorisierung der Kanäle.",
    },
    {
      title: "Immobilienmakler Regional",
      slug: "immobilienmakler-regional",
      excerpt: "Starke Meta- und Content-Impulse, die sich auch für junge Unternehmen übertragen lassen.",
    },
  ],
  process: [
    { step: 1, title: "Kick-off", description: "Ziele, Ist-Zustand und Engpässe Ihres Marketings klären." },
    { step: 2, title: "Positionierung", description: "Zielgruppe, Angebot und Botschaften schärfen." },
    { step: 3, title: "Kanal-Fokus", description: "Die sinnvollsten Kanäle für Ihren Start festlegen." },
    { step: 4, title: "Umsetzungsplan", description: "Konkrete Maßnahmen für die nächsten 30–90 Tage ableiten." },
    { step: 5, title: "Nächste Schritte", description: "Offene Fragen klären und optionalen Follow-up besprechen." },
  ],
  faq: [
    {
      question: "Für wen ist der Startup-Workshop gedacht?",
      answer:
        "Für Gründerinnen, Gründer und junge Unternehmen, die Marketing brauchen, aber noch kein großes Budget für eine laufende Betreuung haben.",
    },
    {
      question: "Wie lange dauert der Workshop?",
      answer: "In der Regel fünf bis sechs Stunden – kompakt, fokussiert und mit klaren Ergebnissen am Ende.",
    },
    {
      question: "Was nehme ich mit?",
      answer:
        "Klarheit zu Zielgruppe und Positionierung, priorisierte Kanäle und einen konkreten Umsetzungsplan für die nächsten Wochen.",
    },
    {
      question: "Ersetzt der Workshop eine Agentur-Betreuung?",
      answer:
        "Nein – er ist der starke Einstieg. Viele starten damit selbst und kommen später für Ads, Content oder Website zurück.",
    },
    {
      question: "Können wir den Workshop remote machen?",
      answer: "Ja, vor Ort oder online – je nachdem, was für Ihr Team besser passt.",
    },
  ],
};

export const marketingSponsoringVereine: LandingPage = {
  slug: "marketing-sponsoring-vereine",
  path: "/leistungen/workshops/marketing-sponsoring-vereine",
  category: "workshops",
  meta: {
    title: "Marketing & Sponsoring Workshop für Vereine",
    description:
      "Praxis-Workshop für Vereine zu Marketing und Sponsoring: Sichtbarkeit aufbauen, Mitglieder gewinnen und Sponsoren überzeugend ansprechen.",
    keywords: [
      "marketing workshop vereine",
      "sponsoring workshop verein",
      "vereinsmarketing",
      "sponsoren gewinnen verein",
    ],
  },
  hero: {
    headline: "Marketing & Sponsoring für Vereine – Workshop mit Praxisbezug",
    subheadline:
      "Mitglieder, Sichtbarkeit und Sponsoren gehören zusammen. In einem kompakten Workshop entwickeln wir, wie Ihr Verein online überzeugt – und Sponsoren klarer anspricht.",
  },
  problem: {
    title: "Viele Vereine kennen das",
    points: [
      "Gute Arbeit im Verein – aber online kaum sichtbar",
      "Mitgliedergewinnung läuft vor allem über Mundpropaganda",
      "Sponsorenanfragen wirken unklar oder wenig professionell",
      "Keine Zeit und kein Budget für ein volles Marketing-Team",
      "Social Media passiert sporadisch, ohne Plan und ohne Wirkung",
    ],
  },
  solution: {
    title: "Marketing und Sponsoring als ein System",
    content:
      "In diesem Workshop verbinden wir Vereinsmarketing mit Sponsoring: Wir schärfen Ihren Auftritt, priorisieren Kanäle und Inhalte und zeigen, wie Sie Reichweite, Community und Sponsoren-Nutzen glaubwürdig darstellen. Kompakt, praxisnah und auf Vereine zugeschnitten – ohne Agentur-Overhead.",
  },
  benefits: [
    {
      title: "Vereins-Know-how",
      description: "Erfahrung mit Sportvereinen, Events und Sponsoring – nah an der Vereinsrealität.",
      icon: "users",
    },
    {
      title: "Mitglieder & Community",
      description: "Klarer Plan, wie Sie neue Mitglieder ansprechen und bestehende binden.",
      icon: "target",
    },
    {
      title: "Sponsoring-Story",
      description: "Wie Sie Reichweite, Zielgruppe und Nutzen so zeigen, dass Sponsoren zuhören.",
      icon: "star",
    },
    {
      title: "Kompakt & umsetzbar",
      description: "In wenigen Stunden Prioritäten und nächste Schritte – statt endloser Theorie.",
      icon: "rocket",
    },
    {
      title: "Social Media mit Sinn",
      description: "Welche Formate wirklich helfen – und was Sie getrost weglassen können.",
      icon: "social",
    },
    {
      title: "Sofort startklar",
      description: "Checklisten und Botschaften, die Ihr Vorstand oder Marketing-Team direkt nutzen kann.",
      icon: "check",
    },
  ],
  references: [
    {
      title: "Regionaler Sportverein",
      slug: "verein-regional",
      excerpt: "Mitgliedergewinnung und Sponsorenakquise über Social Media und klare Positionierung.",
    },
    {
      title: "Golfclub Raffelberg",
      slug: "golfclub-raffelberg",
      excerpt: "Digitale Mitgliedergewinnung und professionelle Online-Präsenz für Clubs und Vereine.",
    },
  ],
  process: [
    { step: 1, title: "Ist-Analyse", description: "Auftritt, Zielgruppen und bisherige Sponsoring-Ansätze prüfen." },
    { step: 2, title: "Positionierung", description: "Stärken, Werte und Nutzen Ihres Vereins klar formulieren." },
    { step: 3, title: "Kanal- & Content-Plan", description: "Die wichtigsten Kanäle und Formate für Mitglieder und Sponsoren festlegen." },
    { step: 4, title: "Sponsoring-Paket", description: "Ansatz für Ansprache, Leistungen und digitale Präsentation entwickeln." },
    { step: 5, title: "Umsetzung", description: "Konkrete To-dos für Vorstand, Marketing und Social Media ableiten." },
  ],
  faq: [
    {
      question: "Für welche Vereine eignet sich der Workshop?",
      answer:
        "Für Sportvereine und andere Organisationen, die Mitglieder gewinnen, Sichtbarkeit aufbauen und Sponsoren professioneller ansprechen wollen.",
    },
    {
      question: "Geht es nur um Social Media?",
      answer:
        "Nein. Social Media ist ein Baustein – genauso wichtig sind Positionierung, Ansprache und wie Sie Sponsoring-Nutzen sichtbar machen.",
    },
    {
      question: "Wie lange dauert der Workshop?",
      answer: "Typischerweise fünf bis sechs Stunden – kompakt und mit klaren Ergebnissen für Ihr Team.",
    },
    {
      question: "Können mehrere Personen aus dem Verein teilnehmen?",
      answer: "Ja, sinnvoll sind oft Vorstand, Marketing-Verantwortliche und ggf. Sponsoring-Ansprechpartner.",
    },
    {
      question: "Was, wenn wir danach mehr Unterstützung brauchen?",
      answer:
        "Dann können wir optional in laufende Betreuung übergehen – etwa Content, Ads oder Mitgliedergewinnung.",
    },
  ],
};

export const fullLandingPages: LandingPage[] = [
  googleAdsFuerHandwerker,
  leadgewinnung,
  mitgliedergewinnungGolf,
  localSeo,
  mitarbeitergewinnung,
  marketingWorkshopStartups,
  marketingSponsoringVereine,
];
